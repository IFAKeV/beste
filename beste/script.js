let data = { persons: [], facilities: [], locations: [], departments: [], roles: [] };
...
function createCard(type, title, details, fullData) {
    ...
    if (type === 'facility' && fullData.departmentId) {
        departmentClass = `department-${fullData.departmentId}`;
    } else if (type === 'person') {
        // Personen können mehreren Facilities angehören, wir wählen den ersten Fachbereich
        if (fullData.facilities && fullData.facilities.length > 0) {
            const facility = data.facilities.find(f => f.id === fullData.facilities[0].facilityId);
            if (facility && facility.departmentId) {
                departmentClass = `department-${facility.departmentId}`;
            }
        }
    }

    card.className = `card ${type}-card ${departmentClass}`; // Department-Klasse hinzufügen
    card.innerHTML = `
    ...
    switch(type) {
        case 'person':
            ...
            if (itemData.facilities && itemData.facilities.length > 0) {
		        ...
		        const facility = data.facilities.find(f => f.id === itemData.facilities[0].facilityId);
		        if (facility && facility.departmentId) {
		            const department = getDepartmentById(facility.departmentId);
                ...
                itemData.facilities.forEach(facilityRef => {
					...
					const roleId = facilityRef.role;
const facilityName = getFacilityName(facilityId);
                    const role = getRoleById(roleId);
                    facilitiesHtml += `<li style="list-style-type: none;">${role.sign} <a href="#" data-type="facility" data-id="${facilityId}">${facilityName}</a> <span style="color:#888">/ ${role.name}</span></li>`;
                });
                facilitiesHtml += '</ul>';
            } else {
			    ...
			    detailsHtml = `<h2>${departmentName}</h2><h1>${itemData.name}</h1>${facilitiesHtml}`;						
            ...
            case 'facility':
                ...
                personsHtml += `<li style="list-style-type: none;">${roleData.sign} <a href="#" data-type="person" data-id="${person.id}">${person.name}</a> <span style="color:#888">/ ${roleData.name}</span></li>`;
            });
            personsHtml += '</ul>';
			    ...
			    detailsHtml = `
			        ...
			        <h2>${department.name}</h2> <!-- Department-Name als Überschrift -->
    ...
    content.classList.remove(
        'department-1', 'department-2', 'department-3', // Fügen Sie alle Department-Klassen hinzu
        'person-modal', 'facility-modal', 'location-modal' // Entfernen Sie auch eventuelle Typ-Klassen
    );
   let departmentId = null;

    if (type === 'facility' && itemData.departmentId) {
     ...
     if (departmentId) {
        content.classList.add(`department-${departmentId}`);
    } else {
        // Falls kein Department vorhanden ist, können Sie eine Standardklasse hinzufügen oder nichts tun
    }
...
function getDepartmentById(id) {
    return data.departments.find(d => d.id === id) || { name: 'Unbekannt', color: '#000000' };
}
